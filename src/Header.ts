...
export const TAGS = {
    ...
    BlackTitle: 'BlackTitle', // These use string values such as "FM", "IM", and "GM"; these tags are used only for the standard abbreviations for FIDE titles. A value of "-" is used for an untitled player.
    WhiteElo: 'WhiteElo',
    BlackElo: 'BlackElo', // These tags use integer values; these are used for FIDE Elo ratings. A value of "-" is used for an unrated player.
    WhiteUSCF: 'WhiteUSCF',
    BlackUSCF: 'BlackUSCF', // These tags use integer values; these are used for USCF (United States Chess Federation) ratings. Similar tag names can be constructed for other rating agencies.
    WhiteNA: 'WhiteNA',
    BlackNA: 'BlackNA:', // These tags use string values; these are the e-mail or network addresses of the players. A value of "-" is used for a player without an electronic address.
    WhiteType: 'WhiteType',
    BlackType: 'BlackType', // These tags use string values; these describe the player types. The value "human" should be used for a person while the value "program" should be used for algorithmic (computer) players.
